...
--traces
uneval (CaseL t m x t1) = Case (uneval t) (Match (x,uneval t1) (bot, Hole))
uneval (CaseR t m x t2) = Case (uneval t) (Match (bot,Hole) (x, uneval t2))
uneval (IfThen t e1 e2 t1) = If (uneval t) (uneval t1) Hole
uneval (IfElse t e1 e2 t2) = If (uneval t) Hole (uneval t2)
uneval (Call t1 t2 k t) = App (uneval t1) (uneval t2)
uneval _ = error "Wtf?"
